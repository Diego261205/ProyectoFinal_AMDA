    ...
    <body>
        ...
        <main class="content-container">
            ...
            <div class="report-card no-print">
                ...
                <%
                    String filterAgency = request.getParameter("filtro_agencia");
                    String filterMov = request.getParameter("filtro_movimiento");
                    String filterStatus = request.getParameter("filtro_estatus");
                    String filterOrder = request.getParameter("filtro_orden");

                    if (filterAgency == null) filterAgency = "TODAS";
                    if (filterMov == null) filterMov = "TODOS";
                    if (filterStatus == null) filterStatus = "TODOS";
                    if (filterOrder == null) filterOrder = "DEFAULT";
                %>
                <form action="reportes.jsp" method="GET" class="filter-grid">
                    <div>
                        ...
                        <select name="filtro_agencia">
                            <option value="TODAS" <%= "TODAS".equals(filterAgency) ? "selected" : "" %>>-- Todas las Agencias --</option>
                            <option value="Ford Country" <%= "Ford Country".equals(filterAgency) ? "selected" : "" %>>Ford Country</option>
                            <option value="Honda Campestre" <%= "Honda Campestre".equals(filterAgency) ? "selected" : "" %>>Honda Campestre</option>
                            <option value="Mercedes-Benz Aguascalientes" <%= "Mercedes-Benz Aguascalientes".equals(filterAgency) ? "selected" : "" %>>Mercedes-Benz Aguascalientes</option>
                            <option value="Audi Aguascalientes" <%= "Audi Aguascalientes".equals(filterAgency) ? "selected" : "" %>>Audi Aguascalientes</option>
                            <option value="Eurocentro Camionero" <%= "Eurocentro Camionero".equals(filterAgency) ? "selected" : "" %>>Eurocentro Camionero</option>
                            <option value="Autodistribuidores del Centro" <%= "Autodistribuidores del Centro".equals(filterAgency) ? "selected" : "" %>>Autodistribuidores del Centro</option>
                            <option value="Chevrolet Herrera Motors" <%= "Chevrolet Herrera Motors".equals(filterAgency) || "Chevrolet".equals(filterAgency) || "Herrera Motors".equals(filterAgency) ? "selected" : "" %>>Chevrolet Herrera Motors</option>
                            <option value="Nissan Torres Corzo" <%= "Nissan Torres Corzo".equals(filterAgency) ? "selected" : "" %>>Nissan Torres Corzo</option>
                            <option value="Toyota Aguascalientes" <%= "Toyota Aguascalientes".equals(filterAgency) ? "selected" : "" %>>Toyota Aguascalientes</option>
                            <option value="BMW Aguascalientes" <%= "BMW Aguascalientes".equals(filterAgency) ? "selected" : "" %>>BMW Aguascalientes</option>
                        </select>
                    </div>
                    <div>
                        <label style="margin-top: 0;">Tipo de Trámite:</label>
                        <select name="filtro_movimiento">
                            <option value="TODOS" <%= "TODOS".equals(filterMov) ? "selected" : "" %>>-- Todos los Trámites --</option>
                            <option value="Placa Nueva" <%= "Placa Nueva".equals(filterMov) ? "selected" : "" %>>Placa Nueva</option>
                            <option value="Permiso de Circulacion" <%= "Permiso de Circulacion".equals(filterMov) ? "selected" : "" %>>Permiso de Circulación</option>
                        </select>
                    </div>
                    <div>
                        <label style="margin-top: 0;">Estatus del Pago:</label>
                        <select name="filtro_estatus">
                            <option value="TODOS" <%= "TODOS".equals(filterStatus) ? "selected" : "" %>>-- Todos los Estatus --</option>
                            <option value="PAGADO" <%= "PAGADO".equals(filterStatus) ? "selected" : "" %>>Solo Pagados</option>
                            <option value="PENDIENTE" <%= "PENDIENTE".equals(filterStatus) ? "selected" : "" %>>Solo Pendientes</option>
                        </select>
                    </div>
                    <div>
                        <label style="margin-top: 0;">Ordenar por:</label>
                        <select name="filtro_orden" id="ordenarPor" onchange="sortTable();">
                            <option value="DEFAULT" <%= "DEFAULT".equals(filterOrder) ? "selected" : "" %>>Orden de registro (Recientes primero)</option>
                            <option value="ANTIGUEDAD_ASC" <%= "ANTIGUEDAD_ASC".equals(filterOrder) ? "selected" : "" %>>Fecha de trámite (Antiguos primero)</option>
                            <option value="ANTIGUEDAD_DESC" <%= "ANTIGUEDAD_DESC".equals(filterOrder) ? "selected" : "" %>>Fecha de trámite (Recientes primero)</option>
                            <option value="CLIENTE_AZ" <%= "CLIENTE_AZ".equals(filterOrder) ? "selected" : "" %>>Cliente / Agencia (A-Z)</option>
                            <option value="CLIENTE_ZA" <%= "CLIENTE_ZA".equals(filterOrder) ? "selected" : "" %>>Cliente / Agencia (Z-A)</option>
                            <option value="VEHICULO_AZ" <%= "VEHICULO_AZ".equals(filterOrder) ? "selected" : "" %>>Marca (A-Z)</option>
                        </select>
                    </div>
                    ...
                    <tbody>
                        <%
                            ...
                            try {
                                ...
                                if (!filterAgency.equals("TODAS")) {
                                    if ("Chevrolet Herrera Motors".equals(filterAgency)) {
                                        sql += " AND (s.cliente_nombre = ? OR s.cliente_nombre = 'Chevrolet' OR s.cliente_nombre = 'Herrera Motors')";
                                    } else {
                                        sql += " AND s.cliente_nombre = ?";
                                    }
                                }
                                if (!filterMov.equals("TODOS")) {
                                    sql += " AND s.tipo_movimiento = ?";
                                }
                                if (filterStatus.equals("PAGADO")) {
                                    sql += " AND s.pagado = TRUE";
                                } else if (filterStatus.equals("PENDIENTE")) {
                                ...
                                int paramIdx = 1;
                                if (!filterAgency.equals("TODAS")) {
                                    ps.setString(paramIdx++, filterAgency);
                                }
                                if (!filterMov.equals("TODOS")) {
                                    ps.setString(paramIdx++, filterMov);
                                }
