...
<%
    ...
    try {
        ...
        if (rs.next()) {
            ...
            ownerName = rs.getString("nombre_completo");
            edad = rs.getInt("edad");
            birthDate = rs.getString("fecha_nacimiento");
            phone = rs.getString("telefono");
            address = rs.getString("domicilio");
            movType = rs.getString("tipo_movimiento");
            clientRequest = rs.getString("cliente_nombre");
            marca = rs.getString("marca");
            modelo = rs.getString("modelo");
            vin = rs.getString("vin");
            plateType = rs.getString("tipo_placa");
            amount = rs.getDouble("importe");
        } else {
    ...
    <body>
        ...
        <div class="content-container">
            <div class="form-container">
                <h2><i class="fa-solid fa-pen-to-square"></i> Modificar Trámite Vehicular (ID: <%= serviceId %>)</h2>
                <form action="actualizar_servicio.jsp" method="POST">
                    <!-- Enviamos los IDs ocultos para saber qué registros actualizar -->
                    <input type="hidden" name="id_servicio" value="<%= serviceId %>">
                    <input type="hidden" name="id_dueno" value="<%= ownerId %>">

                    <div class="grid-form">
                        <section class="form-section">
                            <h3><i class="fa-solid fa-address-card"></i> Datos del Dueño</h3>
                            <label>Nombre Completo:</label>
                            <input type="text" name="nombre_dueno" value="<%= ownerName %>" required>
                            <label>Fecha de Nacimiento:</label>
                            <input type="date" name="nacimiento" value="<%= birthDate %>" required>

                            <label>Teléfono:</label>
                            <input type="text" name="telefono" value="<%= phone %>" required>

                            <label>Domicilio:</label>
                            <input type="text" name="domicilio" value="<%= address %>" placeholder="Busca la dirección con Google Maps..." required autocomplete="off">
                        ...
                        <section class="form-section">
                            ...
                            <select name="tipo_movimiento">
                                <option value="Placa Nueva" <%= "Placa Nueva".equals(movType) ? "selected" : "" %>>Placa Nueva</option>
                                <option value="Permiso de Circulacion" <%= "Permiso de Circulacion".equals(movType) ? "selected" : "" %>>Permiso de Circulación</option>
                            </select>

                            <label>Cliente / Agencia que solicita:</label>
                            <select name="cliente_solicita">
                                <option value="Ford Country" <%= "Ford Country".equals(clientRequest) ? "selected" : "" %>>Ford Country</option>
                                <option value="Honda Campestre" <%= "Honda Campestre".equals(clientRequest) ? "selected" : "" %>>Honda Campestre</option>
                                <option value="Mercedes-Benz Aguascalientes" <%= "Mercedes-Benz Aguascalientes".equals(clientRequest) ? "selected" : "" %>>Mercedes-Benz Aguascalientes</option>
                                <option value="Audi Aguascalientes" <%= "Audi Aguascalientes".equals(clientRequest) ? "selected" : "" %>>Audi Aguascalientes</option>
                                <option value="Eurocentro Camionero" <%= "Eurocentro Camionero".equals(clientRequest) ? "selected" : "" %>>Eurocentro Camionero</option>
                                <option value="Autodistribuidores del Centro" <%= "Autodistribuidores del Centro".equals(clientRequest) ? "selected" : "" %>>Autodistribuidores del Centro</option>
                                <option value="Chevrolet Herrera Motors" <%= "Chevrolet Herrera Motors".equals(clientRequest) || "Chevrolet".equals(clientRequest) || "Herrera Motors".equals(clientRequest) ? "selected" : "" %>>Chevrolet Herrera Motors</option>
                                <option value="Nissan Torres Corzo" <%= "Nissan Torres Corzo".equals(clientRequest) ? "selected" : "" %>>Nissan Torres Corzo</option>
                                <option value="Toyota Aguascalientes" <%= "Toyota Aguascalientes".equals(clientRequest) ? "selected" : "" %>>Toyota Aguascalientes</option>
                                <option value="BMW Aguascalientes" <%= "BMW Aguascalientes".equals(clientRequest) ? "selected" : "" %>>BMW Aguascalientes</option>
                            </select>
                            ...
                            <input type="text" name="tipo_placa" value="<%= plateType %>">

                            <label>Importe a Cobrar ($):</label>
                            <input type="number" step="0.01" name="importe" value="<%= amount %>" required>
                        </section>
                    </div>
